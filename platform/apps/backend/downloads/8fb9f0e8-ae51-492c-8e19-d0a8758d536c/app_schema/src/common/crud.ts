...
export const putMessageBatch = async (
  batch: Array<{
    user_id: string;
    role: 'user' | 'assistant';
    content: string | Array<ContentBlock>;
  ...
  await db
    ...
    .values(
      batch.map(({ user_id, role, content }) => ({ user_id, role, content }))
    );
};
...
export const getHistory = async (
  ...
  const rows = await db
    ...
    .where(
      and(
        eq(internalMessagesTable.user_id, user_id),
        role ? eq(internalMessagesTable.role, role) : undefined
      )
    )
