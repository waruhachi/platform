...
export const InfiniteFreeText = (props) => {
    ...
    return (_jsxs(Box, { flexDirection: "column", gap: 1, width: "100%", children: [errorInputs.map((input, index) => (_jsx(FreeTextError, { ...input }, index))), _jsx(FreeText, { ...props, onSubmitError: onSubmitError, status: freeInputStatus })] }));
};
export const FreeText = (props) => {
    const { question, onSubmit, placeholder, status, loadingText, onSubmitError, } = props;
