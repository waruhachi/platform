...
function MockedAgentAppScreen() {
    ...
    console.log({ data });
    return (_jsxs(Box, { children: [_jsx(Text, { children: "Mocked Agent App" }), _jsx(InfiniteFreeText, { successMessage: "Changes applied successfully", question: "How would you like to modify your application?", placeholder: "e.g., Add a new feature, modify behavior, or type 'exit' to finish", onSubmit: (text) => startBuilding('hey'), status: status, errorMessage: error?.message, loadingText: "Applying changes...", retryMessage: "Please retry." })] }));
}
